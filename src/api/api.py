...
async def get_sitemap() -> Response:
    ...
    last_modified = datetime(2025, 5, 8, 16, 3, 0)

    sitemap = Sitemap(domain="https://dronperminov.ru")
    sitemap.add_url(url="", last_modified=last_modified, priority=1.0)
    sitemap.add_url(url="json-formatter", last_modified=last_modified, priority=0.9)
    ...
    for article in articles:
        sitemap.add_url(url=f"articles/{article.link}", last_modified=article.datetime, priority=0.7)

    for post in posts:
        sitemap.add_url(url=f"post/{post.post_id}", last_modified=post.timestamp, priority=0.5)
